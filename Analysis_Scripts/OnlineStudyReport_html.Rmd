...
Regardless of outlier removal, we had decided that I would calculate the difference between T3-T2 fluency for each language and category seperately (but averaging over the two "categories" per language). The second plot below shows how these difference scores correlate with one another. 

```{r fluency, echo = FALSE, warning=FALSE}
setwd("//cnas.ru.nl/wrkgrp/STD-OnlineStudy_DataCoding")
...
corrmat <- fluency[,c(2,3,4,8,9,10,12,13)]
...
Participants completed the same motivation questionnaire at all time points. The questionnaire consists of 5 subparts. Below I'm plotting how scores on each of those subparts correlated with each other and across time points (only including T2 and T3).

Based on this plot, I propose to group together 'interest in learning FL in general', 'motivation to learn Spanish in particular' and 'attitude towards the Spanish people & culture' into one score that I will call 'integrative motivation'. The other two subscores appear to correlated less well with the others and so I proposeto keep them seperate.  

<p style="color:tomato"> Predictor for model: 
So far, I calculated the average across T2 and T3 for each subscore and used those as predictors, rather than the difference between T2 and T3. This made most sense to me intuitively. But alternatively, we could only at T2 or only at T3 motivation, or at the difference between the two (how much did motivation change from T2 to T3). The latter makes little sense to me, and I thought an average of the two sessions would be most representative of participants' overall motivation during the time period under investgiation here. But let's discuss this. Overall, you can see that motivation scores at the two time points correlated positively, so someone who was motivated at T2, tended to stay motivated at T3.</p> 
...
Either way, we haven't yet agreed on how to include this predictor in the analyses. My suggestion was to simply add it to the mixed model with T2 and T3 data and session as a predictor, like we do for all other predictors, but there were concerns as to whether that's statistically possible. I think it is, and R doesn't seem to complain when running it either, but I might be missing something here. So let's discuss this once more. 
...
```{r all_pred, echo = FALSE}

## subset to all variables for model
corrmat <- df[,c(2, 120,121,122, 141:150)]
...
How much of the above needs to go in the paper? Do I need to motivate why I calculated differences for some and averages for others in the paper itself, or is that better to put in the supplements? 

What to do about the T2 performance predictor? As you will see further below, we don't have enough power to estimate that many predictors anyway, and I think it's (next to Spanish AOA) one of the first predictors that can go. Also we are still not fully sure how to analyse it. Can I include this in the model just as any of the other predictors, or would I need to run a seperate model for this? Given that it's by far the most common and robust finding in the literature that ultimate L2 attainment prior to attrition onset is a major predictor for forgetting rates, I'm hesistant to ignore it, but I also think we need to compromise given our sample size. Our T2 measure is of course not really a measure of ultimate attainment, so it's questionable to what extend it can be used to get an answer to this question anyway, but it would be weird not to at least look at it. We could alternatively use the T2 self-rating as a measure of ultimate attainment (but this predictor would be highly correlated with the other self-ratings measure, so they couldn't go into one model, am I right?). 

What to do about the motivation scores? See point above.

As you will see further below, for the main analysis with only 96 participants, we don't have enough power to test all the above predictors. The model simply no longer converges with more than 8 predictors. What is our approach to that problem? Do we narrow the main analysis down to only a few of those? Which ones? Do we run subanalyses for some of them? 

# Spanish knowledge over time
...
In what follows, I ran mixed effect models on the Spanish (partial) accuracy scores from both T2 and T3 as DV and session as predictor.

<center>Accuracy ~ Session + (1|PP) + (1|Item)</center>

Model outcomes from such a model can be interpreted as follows:
...
```{r model, results ='asis', message = FALSE, warning = FALSE, echo = FALSE}
...
dffull <- merge(dffull, fluency, by = "ppn")

glmer(cbind(Corr, Incorr)  ~ 
