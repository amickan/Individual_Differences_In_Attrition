...
for (i in 1:length(reg[,1])){  
  k <- 1
  #if (is.na(reg$T1done[i]) == 0 && ((reg$T1done[i] == 1 && (is.na(reg$T2done[i]) == 1 && reg$T2Datum[i] > Sys.Date()+14)))) { 
  if (reg$T1done[i] == 1 && (is.na(reg$T2done[i]) == 1 && reg$T2Datum[i] > Sys.Date()+14)) { 
    if (((is.na(reg$Mail1Sent[i]) == 1) &&
      ...
      ((is.na(reg$Mail7Sent[i]) == 1) &&
       ((reg$compLime1[i] == (Sys.Date()-210) |
         reg$compLime1[i] == (Sys.Date()-209) |
         reg$compLime1[i] == (Sys.Date()-211))))){
...
## Invites to late comers: the people that had no T1 recordings, but were still invited to participate later 
temp <- matrix(NA, 1,ncol(reg))
colnames(temp) <- colnames(reg)
for (i in 1:length(reg[,1])){  
  k <- 1
  if (reg$T1done[i] == 2 && (is.na(reg$T2done[i]) == 1 && reg$T2Datum[i] > Sys.Date()+14)) { #|| (reg$T1done[i] == 2 && is.na(reg$T2done[i]) == 0 && Sys.Date() == as.Date(reg$completedLimeT2[i])+30))){
    if (((is.na(reg$Mail1Sent[i]) == 1) &&
         ((reg$compLime1[i] == (Sys.Date()-91) | 
           reg$compLime1[i] == (Sys.Date()-90) | 
           reg$compLime1[i] == (Sys.Date()-89)))) ||
...
# run the above mail sending stuff again
if (length(temp)==0){print("No invites sent.")} else{
  for (i in 1:length(temp[,1])){
    num <- which(tolower(as.character(reg$tid)) == tolower(as.character(temp$tid[i])))
    if (is.na(temp$Mail1Sent[i]) == T){
      html_body <- paste("<html> Liebe(r)",
                         as.character(temp$firstname[i]),
                         ", <br /><p>Schön, dass du wieder dabei bist. :-) Wir möchten dich nun bitten, zunächst eine kurze Frage zu deinem <b>aktuellen</b> Sprachgebrauch zu beantworten (die erste einer
                         Reihe solcher Emails über die nächsten Monate.</p>
                         <p>Es sollte dich nicht mehr als 2 Minuten kosten, und sollte möglichst zeitnah beantwortet werden, am besten sofort, oder <b>spätestens in den nächsten 3 Tagen</b>.</p> 
                         Hier der Link:</b> <br / >",
                         temp$LimeLink2[i], 
                         "</p><p>Viel Spaß und beste Grüße,<br />
...
for (i in 1:length(reg[,1])){  
  ...
  if (is.na(reg$T2done[i]) == 0 && (reg$T2done[i] == 1 && reg$T3Datum[i] > Sys.Date()+14)) {
    if (((is.na(reg$Mail3Sent[i]) == 1 || (is.na(reg$Mail4Sent[i]) == 1 && (reg$Mail3Sent[i] < Sys.Date()-14))) &&
        ...
        ((is.na(reg$Mail4Sent[i]) == 1 || (is.na(reg$Mail5Sent[i]) == 1 && reg$Mail4Sent[i] < Sys.Date()-14)) &&
         ((reg$completedLimeT2[i] == (Sys.Date()-59) | 
           reg$completedLimeT2[i] == (Sys.Date()-61) | 
           reg$completedLimeT2[i] == (Sys.Date()-60) |
           reg$completedLimeT2[i] == (Sys.Date()-90) |
        ...
        ((is.na(reg$Mail5Sent[i]) == 1 || (is.na(reg$Mail6Sent[i]) == 1 && reg$Mail5Sent[i] < Sys.Date()-14)) &&
         ((reg$completedLimeT2[i] == (Sys.Date()-89) | 
           reg$completedLimeT2[i] == (Sys.Date()-90) | 
           reg$completedLimeT2[i] == (Sys.Date()-91) |
           reg$completedLimeT2[i] == (Sys.Date()-120) |
        ...
        ((is.na(reg$Mail6Sent[i]) == 1 || (is.na(reg$Mail7Sent[i]) == 1 && reg$Mail6Sent[i] < Sys.Date()-14)) &&
         ((reg$completedLimeT2[i] == (Sys.Date()-120) | 
           reg$completedLimeT2[i] == (Sys.Date()-119) | 
           reg$completedLimeT2[i] == (Sys.Date()-121) |
           reg$completedLimeT2[i] == (Sys.Date()-150) |
        ...
        ((is.na(reg$Mail7Sent[i]) == 1 || (is.na(reg$Mail8Sent[i]) == 1 && reg$Mail7Sent[i] < Sys.Date()-14)) &&
         ((reg$completedLimeT2[i] == (Sys.Date()-151) | 
           reg$completedLimeT2[i] == (Sys.Date()-149) | 
           reg$completedLimeT2[i] == (Sys.Date()-150) |
           reg$completedLimeT2[i] == (Sys.Date()-180) |
        ...
        ((is.na(reg$Mail8Sent[i]) == 1 || (is.na(reg$Mail9Sent[i]) == 1 && reg$Mail8Sent[i] < Sys.Date()-14)) &&
         ((reg$completedLimeT2[i] == (Sys.Date()-181) |
           reg$completedLimeT2[i] == (Sys.Date()-179) |
           reg$completedLimeT2[i] == (Sys.Date()-180) |
        ...
        ((is.na(reg$Mail9Sent[i]) == 1 || (is.na(reg$Mail10Sent[i]) == 1 && reg$Mail9Sent[i] < Sys.Date()-14)) &&
         ((reg$completedLimeT2[i] == (Sys.Date()-209) |
           reg$completedLimeT2[i] == (Sys.Date()-210) |
           reg$completedLimeT2[i] == (Sys.Date()-211) |
           reg$completedLimeT2[i] == (Sys.Date()-240) |
        ...
        ((is.na(reg$Mail10Sent[i]) == 1 || (is.na(reg$Mail11Sent[i]) == 1 && reg$Mail10Sent[i] < Sys.Date()-14)) &&
         ((reg$completedLimeT2[i] == (Sys.Date()-239) |
           reg$completedLimeT2[i] == (Sys.Date()-240) |
           reg$completedLimeT2[i] == (Sys.Date()-241) |
           reg$completedLimeT2[i] == (Sys.Date()-270) |
        ...
        ((is.na(reg$Mail11Sent[i]) == 1 || (is.na(reg$Mail12Sent[i]) == 1 && reg$Mail11Sent[i] < Sys.Date()-14)) &&
         ((reg$completedLimeT2[i] == (Sys.Date()-269) |
           reg$completedLimeT2[i] == (Sys.Date()-270) |
           reg$completedLimeT2[i] == (Sys.Date()-271) |
           reg$completedLimeT2[i] == (Sys.Date()-300) |
